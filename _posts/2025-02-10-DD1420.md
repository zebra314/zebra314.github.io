...
Also known as Shannon information, measures how surprising an event is. Note that when the event will definitely happen, the information content is 0, meaning that it is not surprising at all.
...
1. Monotonic
2. $P(x)=1 \implies I(x)=0$
3. $I(x, y) = I(x) + I(y)$

**Information Entropy**

The expected information content of a random variable $X$.
